...
function CreateTodo() {
  ...
  const handleCreateTodo = async (e) => {
    ...
    const user_id = localStorage.getItem('userId'); // Retrieve the stored user ID
    const todoItem = { user_id, title, description, date, time };
    
    try {
      const response = await axios.post('http://localhost:5000/api/todos', todoItem);
